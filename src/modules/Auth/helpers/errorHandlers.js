  ...
  switch (field) {
    case "username":
      ...
      if (error.includes("Ensure this field has at least 3 characters.")) {
        return "Имя пользователя должно содержать от 3 до 15 символов";
      }
    ...
    case "email":
      if (error.includes("user account with this email already exists.")) {
        return "Данный электронный адрес уже занят";
      }
...
export const TokenErrors = (error) => {
  switch (error) {
    case "OTP expired":
      return "Срок действия кода истек, получите новый";
    case "Invalid OTP":
      return "Неверный код";
    case "No OTP token found":
      return "Код не найден";
    case "Invalid request: 'otp_code' parameter is missing.":
      return "Введите код";
